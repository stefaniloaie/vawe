      ...
      <div className="flex items-center gap-1 sm:gap-2 min-w-max">
        {CATEGORY_DEFINITIONS.map(cat => {
          ...
          const content = <>
            ...
            <span>{cat.label}</span>
            {isGame ? (
              <span className="text-[9px] px-1.5 py-0.2 rounded font-black tracking-widest bg-amber-500 text-black uppercase">PLAY</span>
            ) : <>
              {cat.statusBadge === 'LIVE' && <span className={`h-1.5 w-1.5 rounded-full ${isActive ? 'bg-emerald-400' : 'bg-emerald-500/50'}`} />}
              {cat.statusBadge === 'SOON' && <span className="text-[9px] px-1 py-0.2 rounded bg-zinc-900 text-zinc-600 border border-zinc-850">SOON</span>}
              {cat.statusBadge === 'SETUP' && <span className="text-[9px] px-1 py-0.2 rounded border border-amber-500/25 bg-amber-950/20 text-amber-300/80">SETUP</span>}
            </>}
          </>;
          if (!CATEGORY_PATHS[cat.id]) {
            return <button key={cat.id} type="button" onClick={() => onSelectCategory(cat.id)} className={`${className} cursor-pointer opacity-80`} aria-label={`${cat.label} opens the in-app data connector; it is not a public indexable page yet`}>{content}</button>;
          }
          return (
            <a
              key={cat.id}
              id={`cat-nav-${cat.id.toLowerCase()}`}
              href={CATEGORY_PATHS[cat.id]}
